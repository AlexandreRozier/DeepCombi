...
from helpers import char_matrix_to_featmat, generate_name_from_params, postprocess_weights, get_available_gpus
from sklearn.model_selection import ParameterGrid
from parameters_complete import random_state, FINAL_RESULTS_DIR, DATA_DIR, IMG_DIR, nb_of_jobs, real_pnorm_feature_scaling,p_pnorm_filter, filter_window_size, real_top_k, p_svm
from parameters_complete import disease_IDs, diseases, real_Cs, svm_epsilon
...
from combi import  permuted_deepcombi_method, classifier, combi_method, classifier
...
class TestLOTR(object):
    ...
    def test_svm_avg_accuracy(self, real_h5py_data, real_labels, real_idx):
        score = 0
        chroms = [1,2,3,5]
        classifier = svm.LinearSVC(C=real_Cs, penalty='l2', tol=svm_epsilon, verbose=0, dual=True)
        for chrom in tqdm(chroms):
            data = real_h5py_data(chrom)[:]
            x_2d = char_matrix_to_featmat(data, '2d')

            classifier.fit(x_2d[real_idx.train], real_labels[real_idx.train])
            score += classifier.score(x_2d[real_idx.test], real_labels[real_idx.test])
        score /=len(chroms)
    ...
    def test_permutations(self, real_h5py_data, real_labels, real_labels_0based, real_labels_cat, real_idx, alphas, alphas_EV):
        ...
        t_star = permuted_deepcombi_method(model, hp, data, fm, real_labels, real_labels_cat, n_permutations, alpha_sig, real_pnorm_feature_scaling, filter_window_size, real_top_k, mode='min')
        t_star_EV = permuted_deepcombi_method(model,hp, data, fm, real_labels, real_labels_cat, n_permutations, alpha_sig_EV, real_pnorm_feature_scaling, filter_window_size, real_top_k, mode='all')
    ...
    def test_generate_plots(self, real_labels, real_h5py_data):
        ...
        for disease_id in tqdm(disease_IDs):
            ...
            #labels = labels_per_disease[disease_id]
            labels = real_labels # TODO CHANGE
            ...
            def f(c_idx, chromo):
                ...
                model = load_model(os.path.join(FINAL_RESULTS_DIR,'trained_models',disease_id,'model{}.h5'.format(chromo)))
                ...
                top_indices_deepcombi, pp_rm = postprocess_weights(deepcombi_rm,real_top_k, filter_window_size, p_svm, p_pnorm_filter) # TODO check those values
                ...
                combi_pvalues = np.ones(n_snps)
                combi_pvalues[top_indices_combi] = raw_pvalues[top_indices_combi] 


                deepcombi_pvalues = np.ones(n_snps)
                deepcombi_pvalues[top_indices_deepcombi] = raw_pvalues[top_indices_deepcombi] 
                ...
                deepc_selected_pvalues_ax.scatter(range(offset, offset+n_snps), -np.log10(deepcombi_pvalues), c=color, marker='x')
                c_selected_pvalues_ax.scatter(range(offset, offset+n_snps), -np.log10(combi_pvalues), c=color, marker='x')
    ...
    def test_svm_accuracy(self, real_h5py_data, real_labels, real_labels_0based, real_idx):
        ...
        print('Fitting data...' #TODO OFG change svm !!!!! CS MUST BE CHANGED AAAAAAAAAAAAAAAH
    ...
    def test_train_models_with_best_params(self, real_h5py_data, real_labels_cat, real_idx):
        ...
        fm = char_matrix_to_featmat(data, '3d')
        # TODO WARNING LABELS ARE INCORRECT
        for disease_id in disease_IDs:
            ...
            with tensorflow.Session().as_default():
                ...
                model.fit(x=fm[real_idx.train],
                            ...
                            epochs=hp['epochs'],
                            callbacks=[
                                TensorBoard(log_dir=os.path.join(FINAL_RESULTS_DIR,'tb',disease_id,'chrom{}'.format(chrom)),
                                    histogram_freq=10,
                                    write_graph=False,
                                    write_grads=False,
                                    write_images=False)
    ...
    def test_lrp(self, real_h5py_data, real_labels,real_labels_0based,real_labels_cat, real_idx, rep, tmp_path):
        ...
        _, postprocessed_weights = postprocess_weights(
            weights, real_top_k, filter_window_size, p_svm, p_pnorm_filter)
        ...
        _, postprocessed_svm_weights = postprocess_weights(
            np.absolute(svm_weights), real_top_k, filter_window_size, p_svm, p_pnorm_filter)
