...
from parameters_complete import random_state, FINAL_RESULTS_DIR, DATA_DIR, IMG_DIR, nb_of_jobs, real_pnorm_feature_scaling,real_p_pnorm_filter, filter_window_size, real_top_k, p_svm
...
class TestLOTR(object):
    ...
    def test_train_models_with_best_params(self, real_h5py_data, real_labels_cat, real_idx):
        ...
        def train_model_on_disease(real_h5py_data, disease_id, chrom):
            # Load data, hp & labels
        ...
        Parallel(n_jobs=len(disease_IDs))(delayed(train_model_on_disease)(real_h5py_data, disease, chrom) for disease in tqdm(disease_IDs))
    ...
    def test_generate_plots(self, real_labels, real_h5py_data):
        ...
        for disease_id in tqdm(disease_IDs):
            ...
            def f(c_idx, chromo):
                ...
                top_indices_deepcombi, pp_rm = postprocess_weights(deepcombi_rm,real_top_k, filter_window_size, p_svm, real_p_pnorm_filter) # TODO check those values
                ...
                deepc_selected_pvalues_ax.scatter(range(offset, offset+n_snps), -np.log10(deepcombi_selected_pvalues), c=color, marker='x')
                c_selected_pvalues_ax.scatter(range(offset, offset+n_snps), -np.log10(combi_selected_pvalues), c=color, marker='x')
