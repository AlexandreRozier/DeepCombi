...
class TestPipeline(object):
    ...
    def test_read_data(self, real_pvalues):
        ...
        for disease in tqdm(['CD', 'BD']):
            ...
            # Combi
            selected_indices = np.load(os.path.join(FINAL_RESULTS_DIR, 'selected_indices', '{}.npy'.format(disease))).flatten()
           

            # Index-based selection (iloc would be incorrect here!).
            selected_combi_pvalues_peaks = sorted_raw_pvalues_genome_peaks.loc[np.intersect1d(selected_indices,sorted_raw_pvalues_genome_peaks.index)]
            ...
            tp_df = sorted_raw_pvalues_genome_peaks.reset_index().merge(tp_df, on='rs_identifier', how='right').set_index('index')
            tp_df = tp_df.rename(columns={"pvalue_x": "pvalue"}).drop(columns=['pvalue_y'])

            pvalues_labels = pd.Series(np.zeros(len(selected_combi_pvalues_peaks.index)), selected_combi_pvalues_peaks.index)
            pvalues_labels.loc[np.intersect1d(selected_combi_pvalues_peaks.index,tp_df.index)] = 1

            combined_labels = pd.concat([combined_labels, pvalues_labels])
            combined_scores = pd.concat([combined_scores, selected_combi_pvalues_peaks ])

        fpr, tpr, thresholds =  roc_curve(combined_labels.values, -np.log10(combined_scores.pvalue.values) )
        tp = tpr * len(combined_labels.index)
        fp = fpr * len(combined_labels.index)
