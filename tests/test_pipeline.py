    ...
    def test_generate_roc_recall_curves(self, real_pvalues):
        ...
        for disease in tqdm(disease_IDs):
            ...
            tp_df = candidates_raw_pvalues_genome_peaks.reset_index().merge(tp_df,
                                                                            on='rs_identifier',
                                                                            how='right').set_index('index')  # Assign valid index to tp_df
            ...
            selected_combi_pvalues.loc[idx] = candidates_raw_pvalues_genome_peaks.loc[
                idx].pvalue  # Set selected pvalues to 1
        ...
        deepcombi_precision, deepcombi_recall, thresholds = precision_recall_curve(
            combined_labels.values,
            -np.log10(combined_deepcombi_pvalues.values))
        deepcombi_tp = deepcombi_recall * (combined_labels.values == 1).sum()

        rpvt_precision, rpvt_recall, thresholds = precision_recall_curve(combined_labels.values,
                                                                         -np.log10(combined_rpvt_scores.pvalue.values))
